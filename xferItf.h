...
namespace dpu
{
    ...
    /// WEIRD compilation issue when include <memory> and <dpu>, so WORKAROUND, not using smart  pointer right now
    template <typename T>
    class Tensor2d
    {
    public:
        Tensor2d(uint64_t nr_dpus, uint64_t shapes_[]) : nr_dpus(nr_dpus)
        {
            rank = 2;
            shapes[0] = shapes_[0];
            shapes[1] = shapes_[1];
    ...
    public:
        ...
        static uint64_t DpuAllign(uint64_t val)
        {
            return XFER_MEM_ALIGN(val);
        }

        xferItf(const char *buffSymbolName, dpu_set_t &set) : buffOffset(0), buffSymbolName(buffSymbolName), dpu_set(&set), async(false)
        {
            DPU_ASSERT(dpu_get_nr_dpus(set, &nr_dpus));
            flags = async ? DPU_XFER_ASYNC : DPU_XFER_DEFAULT;
        }

        void offsetInc(DescriptorImpl &offset)
        {
            buffOffset += offset;
        }
        ...
        void save()
        {
            savedBuffOffset = buffOffset;
        }

        void restore()
        {
            buffOffset = savedBuffOffset;
        }

        DescriptorImpl offset()
        {
            return buffOffset;
        }
        ...
        template <typename T>
        void scalarBroadcast(SharedObjType objType, T val)
        {
        ...
        template <typename T>
        void vectorBroadcast(SharedObjType objType, std::vector<T> &val)
        {
            auto len = val.size();
            auto *data = val.data();
        ...
        template <typename T>
        void vectorBroadcast(SharedObjType objType, NRvector<T> &val)
        {
            auto len = val.size();
            auto *data = val.data();
            pushDesc(objType);
        ...
        template <typename T>
        void vectorPush(SharedObjType objType, std::vector<std::vector<T>> &val)
        {
            uint32_t each_dpu;
        ...
        template <typename T>
        void vectorGet(std::vector<std::vector<T>> &val)
        {
            uint32_t each_dpu;
        ...
        template <class T>
        void broadcast(T) = delete;
        template <class T, class U>
        void broadcast(T, U) = delete;
        template <class T>
        void get(T) = delete;
        template <class T, class U>
        void get(T, U) = delete;

        void broadcast(Ullong val)
        {
            scalarBroadcast(UllongType, val);
        }

        void broadcast(Uchar val)
        {
            scalarBroadcast(UcharType, val);
        }

        void broadcast(Int val)
        {
            scalarBroadcast(IntType, val);
        }

        template <typename T>
        void push(Tensor2d<T> &t)
        {
            uint32_t each_dpu;
        ...
        void push(std::vector<std::vector<char>> &val)
        {
            vectorPush(CharType, val);
        }

        void push(std::vector<std::vector<Uchar>> &val)
        {
            vectorPush(UcharType, val);
        }

        void push(std::vector<std::vector<Ullong>> &val)
        {
            vectorPush(UllongType, val);
        }

        void push(std::vector<std::vector<Int>> &val)
        {
            vectorPush(IntType, val);
        }

        void broadcast(VecUchar val)
        {
            vectorBroadcast(VecUcharType, val);
        }

        void broadcast(VecInt val)
        {
            vectorBroadcast(VecIntType, val);
        }

        void broadcast(std::vector<std::vector<Int>> &val)
        {
            vectorBroadcast(IntType, val);
        }

        void broadcast(VecUllong val)
        {
            vectorBroadcast(VecUllongType, val);
        }

        void get(std::vector<std::vector<Uchar>> &val)
        {
            vectorGet(val);
        }

        void get(std::vector<std::vector<Ullong>> &val)
        {
            vectorGet(val);
        }
...
void free2dTensor(Tensor2d *t)
{
    buddy_free(t->mram_addr);
}
...
#define XFERITF_INIT()                                        \
    {                                                         \
        .init = xferItf_init,                                 \
        .save = xferItf_save,                                 \
        .restore = xferItf_restore,                           \
        .offsetInc = xferItf_offsetInc,                       \
        .getDesc = xferItf_getDesc,                           \
        .pushDesc = xferItf_pushDesc,                         \
        .getUchar = xferItf_getUchar,                         \
        .getUllong = xferItf_getUllong,                       \
        .getInt = xferItf_getInt,                             \
        .getVecUchar = xferItf_getVecUchar,                   \
        .getVecChar = xferItf_getVecChar,                     \
        .getVecInt = xferItf_getVecInt,                       \
        .getVecUllong = xferItf_getVecUllong,                 \
        .pushVecUchar = xferItf_pushVecUchar,                 \
        .getTensor2dUnsigned = xferItf_getTensor2dUnsigned,   \
        .pushTensor2dUnsigned = xferItf_pushTensor2dUnsigned, \
        .getTensor2dUchar = xferItf_getTensor2dUchar,         \
        .pushTensor2dUchar = xferItf_pushTensor2dUchar,       \
...
void xferItf_save(xferItf *this, uint64_t *reg)
{
    *reg = this->offset;
}
void xferItf_restore(xferItf *this, uint64_t *reg)
{
    this->offset = *reg;
}
...
void xferItf_offsetInc(xferItf *this, DescriptorImpl offset)
{
    this->offset += offset;
}
...
void xferItf_getUchar(xferItf *this, Uchar *val)
{
    SCALAR_FETCH_MRAM_TO_WRAM(val, Uchar);
}

void xferItf_getUllong(xferItf *this, Ullong *val)
{
    SCALAR_FETCH_MRAM_TO_WRAM(val, Ullong);
}
void xferItf_getInt(xferItf *this, Int *val)
{
    SCALAR_FETCH_MRAM_TO_WRAM(val, Int);
}
...
int64_t DpuAllign(uint64_t val)
{
    return XFER_MEM_ALIGN(val);
}
...
#undef MEMORY_ALIGNMENT_BYTE_LOG2
#undef MEMORY_ALIGNMENT_BYTE
#undef XFER_MEM_ALIGN
