/**
 * @file heapscheduler.h
 * @author NC
 * @brief HEDGES inner decoder scoring heap CPU implementation
 *        # original project #
...
template <class T = Doub, class U = void *> struct HeapScheduler {
public:
    ...
    HeapScheduler()
        : bigval(HEAP_SCORE_MAX_VAL_FLOAT)
        , ps(defaultps)
        , ks(0)
        , ar(ps, bigval)
        , br(ps)
    ...
    uint64_t size() { return (uint64_t)(ks); }
