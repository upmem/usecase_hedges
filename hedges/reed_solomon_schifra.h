/**
 * @file reed_solomon_schifra.h
 * @author NC
 * @brief HEDGES project reed solomon encoder/decoder implementation
 * @copyright  Copyright (c) 2000-2018 Arash Partow, All Rights Reserved.
 */
...
template <size_t code_length, size_t fec_length> struct SchifraCode { // see Schifra's example08
    ...
    VecUchar decode(VecUchar &codeword, Int &errs_detected, Int &errs_corrected, Int &err_number, bool &recoverable)
    {
    ...
    VecUchar decode(
        VecUchar &codeword, VecInt &erasures, Int &errs_detected, Int &errs_corrected, Int &err_number, bool &recoverable)
    {
    ...
    b8 decode(b8 codeword, Int &errs_detected, Int &errs_corrected, Int &err_number, bool &recoverable)
    {
        block.reset(); // not needed?
        for (int i = 0; i < code_length; i++)
    ...
    b8 decode(b8 codeword, vector<Ullong> &erasures, Int &errs_detected, Int &errs_corrected, Int &err_number, bool &recoverable)
    {
        block.reset(); // not needed?
        for (int i = 0; i < code_length; i++)
            block.data[i] = static_cast<galois::field_symbol>(codeword.b[i]);
