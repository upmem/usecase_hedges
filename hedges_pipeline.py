...
#
# HEDGES project running on UPMEM DPU for INNER decoder part (and INNER encoder part as option)
# OUTER encoder and OUTER decoder are performed on CPU side                        
...
# Based on original HEDGES project below
...
def dnatomess_dpu(dnapacket, cpu_time):
    ...
    if dpu_profiling:
        ...
        print "[DPU][HEDGES DECODER][PERF][SUMMARY] Dpu Effective Pipeline Eff % \n{:.2f}".format(eff_effective_total)
        ...
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) IO (Eff %) \n{:.2f} ".format(ioem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) push (Eff %) \n{:.2f}  ".format(pushem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) pop (Eff %) \n{:.2f} ".format(popem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) decode (Eff %) \n{:.2f} ".format(decodeem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) hashfunc (Eff %) \n{:.2f} ".format(hashfuncem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) penality (Eff %) \n{:.2f} ".format(penalityem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) hypcompute_first_section (Eff %) \n{:.2f} ".format(hypcompute_first_sectionem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) hypload (Eff %) \n{:.2f} ".format(hyploadem)
        print "[DPU][HEDGES DECODER][PERF][PIPELINE_EFF] (func) hypcompute (Eff %) \n{:.2f} ".format(hypcomputeem)
        ...
        print "[DPU][HEDGES DECODER][TOTAL STRANDSLEN] {:.3f}".format(totstrandlen)
        print "[DPU][HEDGES DECODER][DECODING THROUGPOUT] (dna_sequences/sec/DPU) {:.3f} ".format(dpu_decoding_throughput)
        ...
        if dpu_profiling_csv:
            ...
            ddecoder = [[coderates[coderatecode],
                         hlimit, srate, drate, irate]]
...
for ipacket in range(npackets):
    ...
    print("[CPU] (decoder) packet %3d: (bad Decode : %3d, ereasures : %3d, tot_detect :\
%3d, max_detect : %3d) ( tot_bytes %3d, tot_uncorrect : %3d, max_uncorrect : \
%3d, totercodes : %3d, badbytes : %3d), time [s] %f" % (ipacket,
                                                        baddecodes, erasures, tot_detect, max_detect, tot_bytes, tot_uncorrect,
                                                        max_uncorrect, toterrcodes, badbytes, cpu_time)),

    print("packet OK\r" if badbytes == 0 else "packet NOT ok\r")

    if test_dpu_decoder:
        (dpacket_, dpu_time_) = dnatomess_dpu(
            obspack, cpu_time)  # decode the strands
...
print("[END] (baddecodes %4d, erasures %4d, tot_detect %4d, max_detect %4d) ( totbytes %4d , tot_uncorect %4d, max_uncorect %4d, toterrcodes %4d, badbytes %4d)" % tuple(Totalbads))
tot_badbytes = Totalbads[8]
tot_bytes = Totalbads[4]
print "[END] DNA Perr {} , byteErrorRate {:.2e}".format(Perr, 1.0*tot_badbytes/tot_bytes)
